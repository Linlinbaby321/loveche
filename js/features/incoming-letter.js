...
function ilSaveSettingsFromUI(source) {
    if (typeof settings === 'undefined') return;
    ...
    const minVal = parseInt(document.getElementById('incoming-letter-min-cards')?.value, 10);
    const maxVal = parseInt(document.getElementById('incoming-letter-max-cards')?.value, 10);
    if (isNaN(minVal) || isNaN(maxVal)) return; // spinner 过渡中间值忽略
    settings.incomingLetterMinCards = minVal;
    settings.incomingLetterMaxCards = maxVal;
    ilClampSettings();
    if (source !== 'input') ilSyncSettingsUI();
    ilScheduleNext(true);
    if (typeof throttledSaveData === 'function') throttledSaveData();
}
...
function ilBindSettings() {
    ...
    [
        ...
        if (el && !el.dataset.ilBound) {
            ...
            el.addEventListener('change', () => ilSaveSettingsFromUI('change'));
            el.addEventListener('input', () => ilSaveSettingsFromUI('input'));
        }
    });
    ilSyncSettingsUI();
}
