...
window.deleteMomentsComment = function(postId, commentId) {
    const post = momentsData.posts.find(p => p.id === postId);
    if (!post) return;
    if (!Array.isArray(post.comments)) post.comments = [];
...
function renderMomentsPostCard(post) {
    ...
    ` : '';
    const partnerNote = '';  // 去掉"ta分享了这条朋友圈..."
        ...
        <div class="moments-comments-list">
            ${comments.map(c => {
                ...
                return `<div class="moments-comment-item">
                    <button class="moments-comment-del" onclick="deleteMomentsComment('${post.id}','${c.id}')" title="删除" style="border:none;background:none;color:var(--text-secondary);font-size:11px;cursor:pointer;opacity:0.6;padding:0 2px;float:right;">×</button>
        ...
        <div class="moments-post">
            ...
            <div class="moments-post-body">
                ...
                <div class="moments-post-actions">
                    ...
                    <button class="moments-action-btn" onclick="shareMomentToChat('${post.id}')" title="转发分享">
...
window.shareMomentToChat = function(postId) {
    const post = momentsData.posts.find(p => p.id === postId);
    if (!post) return;
    const name = post.author === 'mine' ? (momentsData.myName || '我') : (momentsData.partnerName || '梦角');
    const text = post.text || '';
    const imgHTML = post.images && post.images.length ? post.images.slice(0, 3).map(u => `<img src="${u}" style="max-width:100%;border-radius:8px;margin-top:4px;">`).join('') : '';
    const card = `<div style="padding:10px;border-radius:12px;background:rgba(var(--accent-color-rgb),0.08);border:1px solid rgba(var(--accent-color-rgb),0.14);"><div style="font-size:12px;opacity:0.7;margin-bottom:4px;">${mEsc(name)}的朋友圈</div>${text ? `<div style="font-size:14px;line-height:1.5;">${mEsc(text)}</div>` : ''}${imgHTML}</div>`;
    if (typeof addMessage === 'function') {
        addMessage({id: Date.now() + Math.random(), sender: 'user', text: card, timestamp: new Date(), status: 'sent', type: 'normal'});
    }
    if (typeof showNotification === 'function') showNotification('朋友圈已分享到聊天', 'success');
};
