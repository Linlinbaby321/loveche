...
function renderPeriodTrackerCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        dayDiv.innerHTML = `<span>${d}</span>${status === 'ovulation' ? '<div class="pt-ovulation-icon">🌸</div>' : ''}`;
        dayDiv.onclick = () => onPeriodDateClick(dateStr);
        dayDiv.ondblclick = (e) => { e.preventDefault(); e.stopPropagation(); onPeriodDateDoubleClick(dateStr); };
...
function onPeriodDateDoubleClick(startStr) {
    if (!ptDoubleClickStart) {
        ptDoubleClickStart = startStr;
        showNotification('已选择起始日期，再次双击结束日期完成标注', 'info');
    } else {
        const endStr = startStr;
        // 确保 start <= end
        let s = ptDoubleClickStart, e = endStr;
        if (s > e) { let t = s; s = e; e = t; }
        const record = { startDate: s, endDate: e };
        periodTrackerData.records.push(record);
        savePeriodTrackerData();
        ptDoubleClickStart = null;
        renderPeriodTrackerCalendar();
        showNotification('经期标注已添加', 'success');
    }
}
