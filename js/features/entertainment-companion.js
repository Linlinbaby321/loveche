...
function emInitDrag() {
    ...
    const move = (clientX) => {
        ...
        const newLane = Math.max(0, Math.min(emState.laneCount - 1, Math.round((left + 20 - laneWidth / 2) / laneWidth)));
        emPlaceAvatars();
        // shoot模式下移动头像到新车道时发射一枚音符向上
        if (emState.running && emState.mode === 'shoot' && newLane !== emState.selfLane) {
            emState.selfLane = newLane;
            emCreateNote('up', emState.selfLane);
        } else {
            emState.selfLane = newLane;
        }
