...
function sendSpellCardMessage() {
    if (typeof settings === 'undefined' || !settings.spellCardModeEnabled) return;
    ...
    while (chosen.length < count && used.size < pool.length) {
        const idx = cftRand(0, pool.length - 1);
        if (used.has(idx)) continue;
        used.add(idx);
        chosen.push(pool[idx]);
    }
    const html = `
        <div class="spell-card-message">
            ${chosen.map(card => `<span class="spell-card-chip">${cftEsc(card).replace(/\n/g, '<br>')}</span>`).join('')}
            <div class="spell-card-tip">${cftPartnerName()}拼了 ${chosen.length} 张主字卡</div>
        </div>
    `;
    cftAddChat('partner', html);
}
...
function saveSpellInputs() {
    if (typeof settings === 'undefined') return;
    settings.spellCardMin = parseInt(document.getElementById('spell-card-min')?.value, 10) || 2;
    settings.spellCardMax = parseInt(document.getElementById('spell-card-max')?.value, 10) || 5;
...
function bindChatFunTools() {
    ...
    ['spell-card-min', 'spell-card-max'].forEach(id => {
        ...
        if (input && !input.dataset.cftBound) {
            ...
            input.addEventListener('change', saveSpellInputs);
            input.addEventListener('input', saveSpellInputs);
        }
    });
