document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        if (window.__freshStartPromise) {
            updateLoader('正在恢复全新状态...', '6%');
            await safeAwait(window.__freshStartPromise, false);
        }
        ...
        if (disclaimerModal) {
            ...
            window._tourSeenAtBoot = !!tourSeen;
            try { localStorage.setItem(APP_PREFIX + 'tourSeenCache', tourSeen ? 'true' : 'false'); } catch (e) {}
...
window.addEventListener('load', function() {
    setTimeout(function() {
        try {
            ...
            document.dispatchEvent(new CustomEvent('daily:greeting-ready'));
