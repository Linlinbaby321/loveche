...
(function() {
    ...
    function enterSite() {
        ...
        document.dispatchEvent(new CustomEvent('splash:entered'));
    }
...
function endTour() {
    ...
    window._tourSeenAtBoot = true;
    try { localStorage.setItem(APP_PREFIX + 'tourSeenCache', 'true'); } catch (e) {}
    document.querySelectorAll('.modal').forEach(m => hideModal(m));
    setTimeout(function() {
        if (typeof window.enterChatPage === 'function') window.enterChatPage();
    }, 900);
}
