...
autoSendInterval: 5,
        ...
        funToolbarExpanded: false,
        redPacketBalance: 0,
        spellCardModeEnabled: false,
        spellCardMin: 2,
        spellCardMax: 5,
        incomingLetterEnabled: false,
        incomingLetterMinHours: 12,
        incomingLetterMaxHours: 36,
        incomingLetterStartTime: '09:00',
        incomingLetterEndTime: '23:00',
        incomingLetterMinCards: 6,
        incomingLetterMaxCards: 12
            };
        }
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    const isFunCardMessage = typeof msg.text === 'string' && (msg.text.includes('chat-red-packet-card') || msg.text.includes('chat-dice-card') || msg.text.includes('redbook-chat-card'));
    const wrapper = document.createElement('div');
    wrapper.className = `message-wrapper ${msg.sender === 'user' ? 'sent' : 'received'}${isFunCardMessage ? ' fun-card-message' : ''}`;
    ...
    if (isFunCardMessage) {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} fun-card-bubble-none`;
    } else if (isImageOnly) {
